...
trains_output_labeled <- readRDS(here::here("rbert", "data", "trains_output_labeled.rds"))
projection_df <- trains_output_labeled %>% 
  dplyr::filter(layer_index %in% c(0, 12))
source(here::here("rbert", "R", "display_pca_dev.R"), local = TRUE)
```
...
trains_output_labeled %>% 
  ...
  ggplot2::theme(legend.position = "none")
```
```{r echo=FALSE, fig.width=15, ref.label="layer-1"}
